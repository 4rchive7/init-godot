...
# ▶ 3개 라인 + 중력 점프(예전 그대로)
# ▶ 좌우 흔들림/회전 없음(정적 비주얼)
# ▶ ⚠️ _set_full_rect 제거: 스택 오버플로우 유발 가능성 있는 헬퍼를 없애고 직접 설정만 사용
# ▶ 씬 없이 코드로 에셋(Texture2D) 적용, 없으면 폴리곤 전투기 폴백
...
func _ready() -> void:
	# 필요하면 여기서 직접 앵커/오프셋 지정 (헬퍼 사용하지 않음)
...
func change_lane(delta_idx: int) -> void:
	...
	# 즉시 스냅하지 말고 목표 y만 갱신 → update에서 부드럽게 이동
...
func update_player(delta: float) -> void:
	...
	if _is_jumping:
		...
		# 점프 중이 아닐 때 라인 타겟으로 스무스 이동 (위/아래 동일)
